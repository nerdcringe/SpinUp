...
void setLeftBase(double speed)
{
  ...
  /*if (PTO.value() == pto_8m_val) { // If pto in - 8 motor
...
{ 
  double Kp = 7; // Proportional makes the speed proportional to the error

  double Ki = 0.02; // Integral accumulates error to the speed over time
  // Integral is often used to to overcome friction at the end due to derivative

  double Kd = 1.45;//45.0; // Derivative slows down the speed if it is too fast
  ...
  if (fabs(fwd_error) > errorThreshold || fabs(turn_derivative) > 0.1) // Check if error is over the acceptable threshold
  {
...
{
  double Kp = 0.4;//0.4;
  double Ki = 0.0001;//0.005;//4;//0.01;
  double Kd = 0.08;//0.31;//33;//43;//1;
  ...
  double errorThreshold = 0.65; // Exit loop when error is less than this

  double speed = 0;

  turn_error = targetDegree - getRotationDeg(); // The error is the relative angle to the target angle

  if (fabs(turn_error) > errorThreshold || fabs(turn_derivative) > 0.8) // only exit if derivative isnt low, meaning speed not changing much
  {
