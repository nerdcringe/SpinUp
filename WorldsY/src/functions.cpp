...
void fwdConst(double inches, double speed, int timeout) {
  double amount = inchesToTicks(inches);
...
void revConst(double inches, double speed, int timeout) {
  double amount = inchesToTicks(inches);
  L1BASE.setTimeout(timeout, msec);
  L2BASE.setTimeout(timeout, msec);
  L3BASE.setTimeout(timeout, msec);
...
void curveL(double inches, double Lspeed, double Rspeed) {
  double amount = inchesToTicks(inches);
  R1BASE.spin(forward, Rspeed, pct);
  R2BASE.spin(forward, Rspeed, pct);
  R3BASE.spin(forward, Rspeed, pct);
  L1BASE.startRotateFor(amount, deg, Lspeed, velocityUnits::pct);
  L2BASE.startRotateFor(amount, deg, Lspeed, velocityUnits::pct);
  L3BASE.rotateFor(amount, deg, Lspeed, velocityUnits::pct);
  R1BASE.stop();
  R2BASE.stop();
  R3BASE.stop();
}
...
void curveR(double inches, double Lspeed, double Rspeed) {
  double amount = inchesToTicks(inches);
  L1BASE.spin(forward, Lspeed, pct);
  L2BASE.spin(forward, Lspeed, pct);
  L3BASE.spin(forward, Lspeed, pct);
  /*R1BASE.startRotateFor(amount, deg, Rspeed, velocityUnits::pct);
  R2BASE.startRotateFor(amount, deg, Rspeed, velocityUnits::pct);
  R3BASE.startRotateFor(amount, deg, Rspeed, velocityUnits::pct);*/
  R1BASE.startRotateFor(amount, deg, Rspeed, velocityUnits::pct);
  R2BASE.startRotateFor(amount, deg, Rspeed, velocityUnits::pct);
  R3BASE.rotateFor(amount, deg, Rspeed, velocityUnits::pct);
  L1BASE.stop();
  L2BASE.stop();
  L3BASE.stop();
}
...
void gyroTurn(double targetAngle, int maxSpeed) {
  ...
  double targetRange = 1.25; // Distance from the desired angle that is allowed
  double error = relativeAngle; // Distance from the desired range
  double progress;              // Degrees the robot has turned already
  double minSpeed = 1.75; // Lowest speed the motors will go; Turning is generally more precise when lower, but slower.
  double deaccelRate = 0.5;//3; // 2.4
  double speed; // Actual speed value of the motors
...
void forwardPID(double targetInches, double maxSpeed, int msTimeout) {
  ...
  double minSpeed = 2.5; // stay at least this fast to overcome friction. May be more reliable than using integral
  double integralPowerLimit = 6 / Ki; // how much speed the integral can contribute
  double integralActiveZone = 6; // How close to error the integral adds up
...
void forwardPIDStraight(double targetInches, double maxSpeed, double targetAngle, int msTimeout) {
  ...
  //double derTheshold = 0.025; // Only exit loop when derivative is less than this
  int settleTimeThreshold = 75; // Only exit loop when within error threshold for this amount of milliseconds
  ...
  while (continueLoop) {
    ...
    // keep straight
      ...
      double cSpeed = angleError * 0.5;
      double maxCSpeed = 2.5;
...
void turnPIDSmall(double targetDeg, double maxSpeed, int msTimeout) {
    ...
    printf("turn  %f \n", targetDeg);
  double Kp = 0.4;
  double Ki = 0.001; // I helps steady state error by slowly increasing speed
  double Kd = 0.0;

  double errorThreshold = 0.75; // Only exit loop when error is less than this
  int settleTimeThreshold = 100; // Only exit loop when within error threshold for this amount of milliseconds
  double integralPowerLimit = 3 / Ki; // How much speed the integral can contribute. Divide by Ki since you  multiply by Ki later
  double integralActiveZone = 5; // How close to error the integral adds up
  ...
  while (continueLoop) {
    ...
    else if (errorThreshold < 1) {errorThreshold += 0.00175;} // if not getting there fast enough, increase the error needed to exit
...
void curvePID(double targetInches, double maxSpeed, double leftMult, int msTimeout) {
  ...
  double errorThreshold = 0.25; // Only exit loop when error is less than this
  //double derTheshold = 0.01; // Only exit loop when derivative is less than this
  int settleTimeThreshold = 50; // Only exit loop when within error threshold for this amount of milliseconds
  double minSpeed = 4; // stay at least this fast to overcome friction. May be more reliable than using integral
  double integralPowerLimit = 5 / Ki; // how much speed the integral can contribute
  double integralActiveZone = 5; // How close to error the integral adds up
