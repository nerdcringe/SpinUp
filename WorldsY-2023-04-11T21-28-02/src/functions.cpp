...
void gyroTurn(double targetAngle, int maxSpeed) {
  ...
  double minSpeed = 1.5; // Lowest speed the motors will go; Turning is generally more precise when lower, but slower.
  double deaccelRate = 1.15;//3; // 2.4
...
void turnPID(double targetDeg, double maxSpeed, int msTimeout) {
    ...
    printf("turn  %f \n", targetDeg);
  double Kp = 0.575;
  double Ki = 0.0045; // I helps steady state error by slowly increasing speed
  double Kd = 0.425;
...
void curvePID(double targetInches, double maxSpeed, double leftMult, int msTimeout) {
  ...
  double errorThreshold = 0.2; // Only exit loop when error is less than this
