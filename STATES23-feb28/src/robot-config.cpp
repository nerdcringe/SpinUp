...
// mini bot
/*
motor L1BASE(PORT13); // front
motor L2BASE(PORT17); // back top

motor R1BASE(PORT18, true);
motor R2BASE(PORT19, true); 


motor L3BASE(PORT2); // back mid
...
triport TriportExt(PORT1); // Get reference for 3 wire extender portss

// PNEUMATICS
digital_out ENDGAME(Triport.A);
digital_out LOCK(Triport.B); //B
...
//inertial INERTIAL(PORT14); // MINIBOT
